...
def test_diff_identical_runs_is_empty() -> None:
    ...
    base = _make_result(fixtures=(_fr("positive/a.md", "positive", Outcome.TRUE_POSITIVE),))
    cur = _make_result(fixtures=(_fr("positive/a.md", "positive", Outcome.TRUE_POSITIVE),))
...
def test_diff_new_false_negative_is_regression() -> None:
    ...
    base = _make_result(fixtures=(_fr("positive/a.md", "positive", Outcome.TRUE_POSITIVE),))
    cur = _make_result(fixtures=(_fr("positive/a.md", "positive", Outcome.FALSE_NEGATIVE),))
    diff = diff_bench(base, cur)
...
def test_diff_new_false_positive_is_regression() -> None:
    ...
    base = _make_result(fixtures=(_fr("negative/b.md", "negative", Outcome.TRUE_NEGATIVE),))
    cur = _make_result(fixtures=(_fr("negative/b.md", "negative", Outcome.FALSE_POSITIVE),))
    diff = diff_bench(base, cur)
    assert diff.is_regression
...
def test_diff_cleared_false_positive_is_improvement_not_regression() -> None:
    ...
    base = _make_result(fixtures=(_fr("negative/b.md", "negative", Outcome.FALSE_POSITIVE),))
    cur = _make_result(fixtures=(_fr("negative/b.md", "negative", Outcome.TRUE_NEGATIVE),))
    diff = diff_bench(base, cur)
    assert not diff.is_regression
...
def test_diff_cleared_false_negative_is_improvement() -> None:
    ...
    base = _make_result(fixtures=(_fr("positive/a.md", "positive", Outcome.FALSE_NEGATIVE),))
    cur = _make_result(fixtures=(_fr("positive/a.md", "positive", Outcome.TRUE_POSITIVE),))
    diff = diff_bench(base, cur)
    assert not diff.is_regression

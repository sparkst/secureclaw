                ...
                if "Edit" in allow_all or "Write" in allow_all:
                    checks.append(
                        PostureCheck(
                            ...
                            status="warning",
                            description=(
                                "Claude Code has blanket file write"
                                " permission. This means AI-generated"
                                " content can modify any file."
                            ),
                            recommendation=(
                                "Consider using 'acceptEdits' mode"
                                " instead of blanket write permission."
                            ),
                ...
                if "Bash" in allow_all:
                    checks.append(
                        PostureCheck(
                            ...
                            status="warning",
                            description=(
                                "Claude Code has blanket shell"
                                " execution permission. Commands"
                                " from prompt injections could"
                                " execute on your system."
                            ),
                            recommendation=(
                                "Review which bash commands are"
                                " allowed. Consider restricting to"
            ...
            if claude_md.exists():
                checks.append(
                    PostureCheck(
                        ...
                        status="secure",
                        description=(
                            f"Found CLAUDE.md at {claude_md}."
                            " This file controls AI behavior"
                            " for this project."
                        ),
                        recommendation=(
                            "Review CLAUDE.md periodically to ensure no unauthorized changes."
                        ),
    ...
    for mcp_path in mcp_paths:
        if mcp_path.exists():
            try:
                ...
                if server_count > 0:
                    recommendation = (
                        "Review each MCP server source. Only use servers from trusted providers."
                        if server_count > 3
                        else "MCP server count is reasonable. Review periodically."
                    )
                    checks.append(
                        PostureCheck(
                            tool_name="Claude Code",
                            ...
                            description=(
                                f"{server_count} MCP server(s)"
                                " configured. Each server extends"
                                " Claude's capabilities with"
                                " external tools."
                            ),
            ...
            if cursorrules.exists():
                checks.append(
                    PostureCheck(
                        ...
                        status="secure",
                        description=(
                            f"Found .cursorrules at"
                            f" {cursorrules}. This file controls"
                            " Cursor AI behavior."
                        ),
                        recommendation=(
                            "Review .cursorrules periodically for unauthorized changes."
                        ),
        ...
        try:
            ...
            checks.append(
                PostureCheck(
                    ...
                    recommendation=(
                        "Review each MCP server. Cursor MCP servers have broad tool access."
                    ),
                    config_path=mcp_path,
                )
            )
    ...
    if skills_dir.exists():
        ...
        checks.append(
            PostureCheck(
                ...
                description=(
                    f"{skill_count} skill(s) installed. Each skill can execute code on your system."
                ),
        ...
        if cron_count > 0:
            checks.append(
                PostureCheck(
                    ...
                    status="warning",
                    description=(
                        f"{cron_count} scheduled task(s)"
                        " running. These execute automatically"
                        " without your confirmation."
                    ),
                    recommendation=(
                        "Review each scheduled task. Ensure they only perform trusted operations."
                    ),
            ...
            if risky_count > 0:
                checks.append(
                    PostureCheck(
                        ...
                        description=(
                            f"Found {risky_count} executable"
                            f" script(s) in ~/{dir_name}."
                            " Downloaded scripts may contain"
                            " prompt injections."
                        ),
                        recommendation=(
                            f"Review scripts in ~/{dir_name}"
                            " before running them or letting"
                            " AI tools access them."
                        ),
            ...
            if p.exists():
                checks.append(
                    PostureCheck(
                        ...
                        status="advisory",
                        description=(
                            f"{browser} profile directory exists"
                            f" at {p}. Browser data (history,"
                            " cached pages) could contain prompt"
                            " injections from malicious websites."
                        ),
                        recommendation=(
                            "Avoid pointing AI tools at browser"
                            " profile directories. If you must,"
                            " scan them first."
                        ),
                        config_path=p,
                    )
